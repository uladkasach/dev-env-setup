...
two guards cap nvim, so no single core can freeze the machine.
...
```
2026-07-22T09:30:01 rss_mb=843 cpu_ticks=12 bufs=7 ts=3 tripped=false cwd=/home/<user>/git/...
```
...
then patch THAT handler at source (see `.common root causes` above). ⚠️ without the log a
diagnosis is a guess — `.behavior/v2026_06_02.fix-nvim-hang` holds two blind attempts.
