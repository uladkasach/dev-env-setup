...
these live in `src/` and take effect once applied through the one entrypoint:

| edit | apply | then |
|---|---|---|
| `zshrc.sh` — the @repo/@branch push | `--what 2.5.zsh --mode apply` | open a new shell |
| `tmux.conf` — the footer | `--what 2.8.tmux --mode apply` | `tmux source-file ~/.tmux.conf` |
| kitty — tab whitespace | `--what 4.3.2.emulator --mode apply` | reload kitty (`ctrl+shift+f5`) |
