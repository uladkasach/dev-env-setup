#!/usr/bin/env bash
######################################################################
# .what = rhx dispatch to the global `git grove del` bash command
#
# .why  = a single allowlistable surface (rhx git.grove.del) so agents
#         can unregister a grove. near-term bridge until git.grove is
#         lifted into the supervisor role.
#
# usage:
#   rhx git.grove.del <name>
######################################################################
set -uo pipefail
