#!/usr/bin/env bash
######################################################################
# .what = rhx dispatch to the global `git grove set` bash command
#
# .why  = a single allowlistable surface (rhx git.grove.set) so agents
#         can register a grove without a raw ssh/git alias prompt.
#         near-term bridge until git.grove is lifted into the supervisor
#         role — see .dream/2026_07_24.git-grove-skills.dream.md
#
# usage:
...
# rhachet forwards its own --skill/--repo/--role flags; drop them so they
# are never mistaken for the grove name or its options.
