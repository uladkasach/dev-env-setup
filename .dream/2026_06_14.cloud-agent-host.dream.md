...
## ✔ .status = LARGELY DELIVERED — the reach half is live; the hibernate half is infra's

this is the umbrella dream the whole `grove` + `duct` vocabulary came out of. what THIS repo
can measure:

| the flow below asks for | what exists here |
|---|---|
| wake the host | `rhx git.grove.wake <grove>` |
| ducts on the host | `rhx duct.open --on duct://<grove>/<tree>/<role>` |
| send tasks | `rhx git.grove.send`, `rhx duct.send` |
| watch from a local window | `src/termwork.sh` + `termwork.test` |
| ssh-over-ssm | `rhx aws.reach.set`, `git.grove.infra.operations` |

⚠️ two clauses this repo CANNOT verify, and must not claim: **auto-hibernate on idle** and the
**cost model**. both live in `ahbode/infrastructure`, so a stamp here would be
`gotcha.my-own-note-became-my-evidence` — ask that repo, never this file.

⚠️ and the flow below is written in a **retired vocabulary** — `agent-host` for what is now a
`grove`, and a `use.agent.host.ts` wish that names no live artifact. read it as the origin
sketch, never as a command to run.
...
duct.open --on duct://agent-host/feature-x/mechanic
duct.open --on duct://agent-host/feature-y/mechanic
duct.open --on duct://agent-host/bugfix-z/mechanic

# 3. send tasks
duct.send --on duct://agent-host/feature-x/mechanic --what "claude --task 'feature X'"
duct.send --on duct://agent-host/feature-y/mechanic --what "claude --task 'feature Y'"
...
export interface DeclaredUnixSshAlias {
   ...
   */
  into: string;  // e.g., 'i-0123456789abcdef0'
...
already support a remote duct — `duct://<host>/<tree>/<role>`. SSH config handles the SSM
