...
#
# usage:
#   zsh emoji.test.zsh                    # tests the widget beside it
#   zsh emoji.test.zsh /path/to/widget    # or an explicit one
#
# guarantee:
...
# `:A` resolves symlinks, so this still finds emoji.zsh when invoked
# through the `rhx emoji.test` symlink in .agent/…/skills/.
...
# stub zle BEFORE the source, so `zle -N` during load is inert too.
# ZLE_CALL records the fall-through target, which is the thing that
# must be right for TAB to stay trustworthy.
typeset -g ZLE_CALL=""
zle() {
  case "$1" in
    -N|-I) return 0 ;;
...
colon() {
  ...
  return 0   # a trailing `[[ ]] &&` would return 1 and trip err_return
}
