...
if [[ ! -s $EMOJI_INDEX ]]; then
  ...
  print -u2 "   fix: grove.provision --what 2.9.emoji --mode apply"
  return 1
fi
...
#      turn is what lets other callers (the test suite) reuse
...
######################################################################
# gesture 2 — the SECOND colon: ':zap:' swaps the instant you type it
#
# slack/github muscle memory. no TAB needed; the second colon IS the
# commit. it fires only when the word under the cursor already matches
# '^:word$', so the colon you type closes one you opened.
#
...
_emoji_colon() {
  ...
  if [[ $word =~ '^:[a-zA-Z0-9_+-]+$' ]]; then
    ...
    if [[ -n $hits ]]; then
      # decisive on purpose: a second colon means the human committed
  ...
  # not a second colon, or no such emoji — type a literal ':'
  zle self-insert
}
